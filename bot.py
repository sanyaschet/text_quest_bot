...
HINTS = [
    """ПОДСКАЗКА 1
Обнаружены признаки внешнего следа, не относящегося к визуальному типу данных. Анализ показывает несоответствие между формой носителя и его содержимым. Источник сигнала может быть связан с памятью места происхождения.""",
    """ПОДСКАЗКА 2
...
Предоставьте источник происхождения зафиксированного следа. Система принимает только идентификатор события, с которым он совпадает. При успешном совпадении будет выдан фрагмент восстановленного сообщения.""",
...
<b>Фрагмент следа из архива восстановлен: Й К К О Д А Д М С Е</b>
